...
//        echo $dato;die();
    ...
    public function get_codprocesojuridico($cod_titulo) {
        $dato = $this->db->query("SELECT DISTINCT CF.COD_CPTO_FISCALIZACION CONCEPTO, RT.PROXIMO_PRESCRIBIR AS PRESCRIPCION, '1' AS PROCEDENCIA, E.CODEMPRESA AS IDENTIFICACION,E.COD_REGIONAL AS REGIONAL, CF.COD_OBLIGACIONCOAC AS OBLIGACION, E.COD_TIPOENTIDAD AS NATURALEZA, TO_CHAR(SYSDATE, 'YY') AS COBRO
            FROM RECEPCIONTITULOS RT,
                  FISCALIZACION F,
                  ASIGNACIONFISCALIZACION AF,
                  EMPRESA E,
            ...
            UNION(
            SELECT DISTINCT TC.COD_TIPOCARTERA,RT.PROXIMO_PRESCRIBIR, '2' AS PROCEDENCIA, E.COD_ENTIDAD,E.COD_REGIONAL, TC.COD_OBLIGACIONCOAC, E.CODTIPOENTIDAD, TO_CHAR(SYSDATE, 'YY') 
            FROM  RECEPCIONTITULOS RT,
                  CNM_CARTERANOMISIONAL NM,
                  CNM_EMPRESA E,
                  TIPOCARTERA TC
            WHERE (RT.COD_CARTERA_NOMISIONAL = NM.COD_CARTERA_NOMISIONAL)
                  ...
                  AND (RT.COD_RECEPCIONTITULO = $cod_titulo)
            UNION(
                SELECT DISTINCT TC.COD_TIPOCARTERA, RT.PROXIMO_PRESCRIBIR, '2' AS PROCEDENCIA, TO_CHAR(E.IDENTIFICACION),E.COD_REGIONAL, TC.COD_OBLIGACIONCOAC, 2, TO_CHAR(SYSDATE, 'YY') 
                FROM RECEPCIONTITULOS RT,
                      CNM_CARTERANOMISIONAL NM,
                      CNM_EMPLEADO E,
                      TIPOCARTERA TC
