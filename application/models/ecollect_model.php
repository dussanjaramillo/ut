...
class Ecollect_model extends CI_Model {
	...
	function obtenerCarteraMisional($NRO_IDENTIFICACION) {
		if(!empty($NRO_IDENTIFICACION)) :
			...
			$this->db->select("CONCEPTOSFISCALIZACION.NOMBRE_CONCEPTO AS DESC_CONCEPTO, "
										...
										. "LIQUIDACION.NUM_LIQUIDACION AS NRO_REFERENCIAPAGO, LIQUIDACION.TOTAL_LIQUIDADO");
			...
			$this->db->select("DECODE(LIQUIDACION.COD_CONCEPTO, 1, 39, 2, 82, 3, 81, 5, 9) AS COD_CONCEPTO", FALSE);
	...
	function obtenerCarteraNoMisional($NRO_IDENTIFICACION) {
		if(!empty($NRO_IDENTIFICACION)) :
			...
			$this->db->select("CNM_CARTERANOMISIONAL.COD_CARTERA_NOMISIONAL, CNM_CUOTAS.NO_CUOTA, CNM_CUOTAS.SALDO_CUOTA, " .
												"CONCEPTORECAUDO.CODIGO_SENA AS COD_CONCEPTO_RECAUDO, CONCEPTORECAUDO.NOMBRE_CONCEPTO, CNM_CARTERANOMISIONAL.CALCULO_MORA, " .
												"CNM_CARTERANOMISIONAL.VALOR_T_MORA, CNM_CARTERANOMISIONAL.TIPO_T_F_MORA, CNM_CARTERANOMISIONAL.TIPO_T_V_MORA, " .
												"CNM_CARTERANOMISIONAL.SALDO_DEUDA");
	...
	function obtenerCuotasAcuerdos($NRO_IDENTIFICACION, $NRO_REFERENCIAPAGO) {
	...
	function TraerFiscalizacion($codigo) {
		if(!empty($codigo)) :
			$this->db->select("TOTAL_CAPITAL, TOTAL_INTERESES, SALDO_DEUDA, SALDO_CAPITAL, SALDO_INTERES, COD_TIPOPROCESO, COD_FISCALIZACION, COD_CONCEPTO");
