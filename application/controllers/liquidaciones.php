<?php
/**
* Archivo para ala administración de los metodos necesarios para las liquidaciones en el proceso administrativo
*
* @packageCartera
            ...
            if ($this -> ion_auth -> logged_in()):
                ...
                if ($this -> ion_auth -> is_admin() || $this -> ion_auth -> in_menu('liquidaciones/consultarLiquidacion')):
                        ...
                        if($expediente == ''):
                            ...
                            $bloqueada = $this -> liquidaciones_model -> consultarLiquidacionBloqueadaFisc($expediente);
                            ...
                            if($bloqueada < 2):
                                ...
                                $resultado = $this -> liquidaciones_model ->  consultarCodigoFiscalizacionBloqueada($expediente,$idusuario);
    ...
    */
                            ...
                            if($diferencia_annos == 0):
                                ...
                                for($fila_int = $mes_fecha_final; $fila_int >= $mes_fecha_inicial; $fila_int --):
                                    ...
                                    if($detalleMensual === TRUE):
                                        ...
                                        throw new Exception('<strong>No ha sido posible cargar el detalle mensual. Error: '. $detalleMensual);
                ...
                if ($this -> ion_auth -> logged_in()): //verificación de acceso autorizado
                    ...
                    if ($this -> ion_auth -> is_admin()  || $this -> ion_auth -> in_menu('liquidaciones/loadFormFic')): //verificación de perfil
                        ...
                        for ($fila = $anno_fecha_inicial; $fila <= $anno_fecha_final; $fila++):
                            ...
                            $cant +=$cant+1;
                            $liquidacion[$fila."_TotalContratoObra"] = (int)str_replace(".", "", $liquidacion[$fila."_TotalContratoObra"]);
                            $liquidacion["totalCosto_".$fila] = (int)str_replace(".", "", $liquidacion["totalCosto_".$fila]);
                            $liquidacion["totalObra_".$fila] = (int)str_replace(".","", $liquidacion["totalObra_".$fila]);
                            if (isset($ContratoCosto))://Valida si existe al menos un contrato
                                if (array_key_exists('tasamensual_' . $fila, $ContratoCosto))://Valida si existe contrato para ese año
                                    $valor_contrato_costo = (int)str_replace(".","",$ContratoCosto['tasamensual_' . $fila]);
                                    $intereses_contrato_costo = (int)str_replace(".","",$ContratoCosto['intereses_' . $fila]);
                                    $aportes_contrato_costo = (int)str_replace(".","",$ContratoCosto['aportes_' . $fila]);
                                    $valorContrato = (int)str_replace(".","",$ContratoCosto['aportes_' . $fila]);
                                else:
    ...
    //                        echo "</pre>";
                            if ($ValorAnual['baseCosto_' . $fila]):
                                ...
                                $valor_anual_costo = (int)str_replace(".","",$ValorAnual['tasamensualcosto_' . $fila]);//Lo que debio haber pagado
                                $BaseCosto = (int)str_replace(".","",$ValorAnual['baseCosto_' . $fila]);
                                ...
                                if($ValorAnual['tasamensualcosto_2014']>0):
                                    ...
                                    $vAnual=((int)str_replace(".","",$ValorAnual['baseCosto_' . $fila])*0.0025);
                            ...
                            $contratoCosto=(int)str_replace(".","",$liquidacion[$fila."_TotalContratoCosto"])+$BaseCosto;
                            var_dump($liquidacion[$fila."_TotalContratoCosto"]);
    ...
    //                         echo "</pre>";
                        ...
                        $liquidacion_fic=array(
                            'nitEmpresa'=>$maestro['NIT_EMPRESA'],
                            'tipoLiquidacion'=>1,
                            'fechaLiquidacion'=>$liquidacion['fechaLiquidacion'],
    ...
    private function ContratoCosto($cantidadContr,$liquidacion,$maestro)
    {
        ...
        for ($contrato = 1; $contrato < $cantidadContr; $contrato++):
                ...
                $tipoFic = 'CONTRATO';
                $aporteMensualAcumulado = 0;
                $SumValorContrato=(int)str_replace(".","",$liquidacion['valor_contr_costo'][$contrato]);
                    ...
                    if (($contr_anno_inicial - $contr_anno_inicial ) == 0):
                        ...
                        if (($contr_mes_final - $contr_mes_inicial) == 0):   //Cuando corresponde al mismo mes
                            if (($contr_dia_final - $contr_dia_inicial) < 31):
                                ...
                                $TotalIntereses = 0;
                                $deudaAcumulada = 0;
                                $valor_mensual = (int)str_replace(".","",$liquidacion['valor_contr_costo'][$contrato]);
                        ...
                        elseif (($contr_mes_final - $contr_mes_inicial) > 0)://Cuando es un mes diferente
                            $valor_mensual =  (int)str_replace(".","",$liquidacion['valor_contr_costo'][$contrato]) / (($contr_mes_final - $contr_mes_inicial) + 1);
                            $deudaAcumulada = 0;
                            $TotalIntereses = 0;
        ...
        $calculos = array();
                                            for($fila = $anno_fecha_final; $fila >= $anno_fecha_inicial; $fila--):
                                                ...
                                                for($emp_mes=1; $emp_mes<=12; $emp_mes++):
                                                    $cant_empl_mes= (int)str_replace(".","",$liquidacion[$fila."_".$emp_mes]);
    ...
    private function ContratoObra($cantidadContr,$liquidacion,$maestro)
    {
        ...
        for ($contrato=1; $contrato<$cantidadContr; $contrato++):
                ...
                $tipoFic = 'CONTRATO';
                $aporteMensualAcumulado = 0;
                $SumValorContrato=(int)str_replace(".","",$liquidacion['valor_contr_obra'][$contrato]);
                //$SumValorContrato; die();
                //Recorro todos los años del periodo del contrato
                for ($c_anno = $contr_anno_final; $c_anno >= $contr_anno_inicial; $c_anno--):
                    //Realizo el calculo para el primer año
                    $TasaMensualAcumulada=0;
                    if (($contr_anno_inicial - $contr_anno_inicial ) == 0):
                        ...
                        if (($contr_mes_final - $contr_mes_inicial) == 0):   //Cuando corresponde al mismo mes
                            if (($contr_dia_final - $contr_dia_inicial) < 31):
                                ...
                                $TotalIntereses = 0;
                                $deudaAcumulada = 0;
                                $valor_mensual = (int)str_replace(".","",$liquidacion['valor_contr_obra'][$contrato]);
                                $tasaMensual = $valor_mensual * 0.0025;
                                if ($contr_mes_final < 10):
                                    $periodo = $contr_anno_final . '-0' . $contr_mes_final;
                                else:
                            ...
                            $detalle = array_merge($detalle, $informacion);
                            elseif (($contr_mes_final - $contr_mes_inicial) > 0)://Cuando es un mes diferente
                            $valor_mensual = (int)str_replace(".","",$liquidacion['valor_contr_obra'][$contrato]) / (($contr_mes_final - $contr_mes_inicial) + 1);
                            $deudaAcumulada = 0;
                            $TotalIntereses = 0;
                        //  $TasaMensualAcumulada=0;
                            for ($i = $contr_mes_inicial; $i <= $contr_mes_final; $i++):
                    ...
                    if ($contr_anno_final - $contr_anno_inicial > 0) ://Cuando el contrato corresponde a varios años
                        $meses=$this->cantidad_meses($contr_anno_inicial,$contr_anno_final,$contr_mes_inicial,$contr_mes_final);
                        $valor_mensual = (int)str_replace(".","",$liquidacion['valor_contr_obra'][$contrato]) / ($meses);
                        $tasaMensualPresAcumulado = 0;
                        $TotalIntereses = 0;
                        $deudaAcumulada = 0;
                        $aporteMensualAcumulado = 0;
    ...
    function consultarTransaccionesFic()
    {
