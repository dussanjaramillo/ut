							...
							foreach($REFERENCE as $ref) :
								...
								switch($ref['COD_CONCEPTO']) :
									...
									case 9	:
										...
										if($ref['COD_CONCEPTO'] == 39)	$datos['COD_SUBCONCEPTO'] = 80;
									...
									case 25 ://93
										...
										foreach($ref['CUOTA'] as $cuota) :
											...
											$datos['COD_CONCEPTO'] = $model->obtenerConcepto($ref['COD_CONCEPTO']);
											...
											if($DatCuota['DIAS_MORA'] > 0) :
												$interes_mora = $apautpago->calcular_mora_nomisional($DatCuota['CALCULO_MORA'], $ref['NRO_REFERENCIAPAGO'], 
																														  $DatCuota['VALOR_T_MORA'], $DatCuota['FECHA_LIM_PAGO'], 
																															$fecha_pago, $cuota['VALOR_CUOTA']);
											else :
												$interes_mora = 0;
											endif;
											$datosNm['INTERES_MORA_GEN'] = $interes_mora;
											if(($cuota['VALOR_CUOTA']-$interes_mora) > $DatCuota['SALDO_INTERES_C'] || $DatCuota['SALDO_INTERES_C'] == 0) :
												$datosNm['SALDO_INTERES_C'] = 0;
											elseif(($cuota['VALOR_CUOTA']-$interes_mora) < $DatCuota['SALDO_INTERES_C']) :
												$datosNm['SALDO_INTERES_C'] = $DatCuota['SALDO_INTERES_C'] - ($cuota['VALOR_CUOTA'] - $interes_mora);
											endif;
											$datosNm['SALDO_AMORTIZACION'] = ($cuota['VALOR_CUOTA'] - $interes_mora - $DatCuota['SALDO_INTERES_C']);
											...
											$datos['DISTRIBUCION_CAPITAL'] = ($cuota['VALOR_CUOTA'] - $interes_mora - $DatCuota['SALDO_INTERES_C']);
											...
											$datos['APLICADO'] = 1;
											//echo "<pre>";print_r($datos);print_r($datosNm);echo "</pre>";exit;
											$aplicado = $apautpago->aplicar_pago($datos);
											if(is_array($aplicado)) :
      ...
      if(!is_null($REFERENCE) and !empty($REFERENCE)) :
				...
				//echo "Imprimiendo reference<pre>";print_r($REFERENCE);exit("</pre>");
					if(is_array($REFERENCE)) :
						...
						$conceptos = array(
							"28" => array("REGIONAL", "REGIONAL_SIIF", "MES", "ANIO", "TIPOEMPRESA", "NRO_TRABAJADORES_PERIODO", "IBC", "DISTRIBUCION_INTERES_MORA", "VALOR_PAGO", "59"),
							...
							"29" => array("REGIONAL", "REGIONAL_SIIF", "NRO_LICENCIA_CONTRATO_OBRA", "NOMBRE_OBRA", "FECHA_INICIO_OBRA", "FECHA_FIN_OBRA", "CIUDAD_OBRA", "TIPO_FIC", "ANIO_PERIODO", "MES_PERIODO", "COSTO_TOTAL_OBRA_TODO_COSTO", "COSTO_TOTAL_MANO_DE_OBRA", "NRO_TRABAJADORES_PERIODO", "DISTRIBUCION_INTERES_MORA", "VALOR_APORTE", "APORTE_SENA", "VALOR_PAGO", "VALOR_TOTAL", "17"),
							"4390" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "100"),
							"4201" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "94"),
							"4203" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "96"),
							"4202" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "95"),
							"4204" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "97"),
							"4305" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "98"),
							"4345" => array("CENTRO_SIIF", "CODIGO_SIIF", "VALOR_PAGO", "VALOR_TOTAL", "99")
						);
						$campos_tabla = array("NITEMPRESA", "PROCEDENCIA", "COD_PROCEDENCIA", "COD_CONCEPTO", "COD_SUBCONCEPTO", "COD_REGIONAL", 
																	...
																	"NRO_LICENCIA_CONTRATO", "NOMBRE_OBRA", "FECHA_INICIO_OBRA", "FECHA_FIN_OBRA", "CIUDAD_OBRA", "TIPO_FIC", 
						...
						if(isset($datos['MES'])) :
							$datos['PERIODO_PAGADO'] = $datos['ANIO']."-".$datos['MES'];
							unset($datos['MES']);
						endif;
						unset($datos['ANIO']);
						$datos['FECHA_PAGO'] = $datos['FECHA_TRANSACCION'] = substr($FECHA_ENTIDADFINANCIERA, 0, 10);
