...
<div id="documentar">
    <input type="hidden" name="documento" id="documento" value="<?php print_r($documento); ?>" readonly=""/>
</div>
<br>
<center>
    <?php
...
<script>
    function limpiaTexto() {
        ...
        var radicado = $("#numero_radicacion").val();
        comentarios = comentarios.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
        radicado = radicado.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
    ...
    $("#push").click(function() {
        ...
        var url5 = "<?php echo base_url("index.php/recepciontitulos/VistaPrevia_Documento"); ?>";
        $("#resultado").load(url5, {cod_coactivo: cod_coactivo}, function() {
            $(".preload, .load").hide();
        });
    });
