...
<div class="caja_negra" id="caja_negra" style="width: 60%; background: #F8F8F8; border-color: black; border: 1px solid grey; margin: auto; overflow: hidden;alignment-adjust: central ;padding: 15px 50px 0">
    ...
    echo form_open_multipart("mcremate/Soporte_Expediente", $attributes);
    ?> 
    <input type="hidden" id="cod_coactivo" name="cod_coactivo" value="<?php echo $cod_coactivo; ?>" readonly> 
    <input type="hidden" id="cod_avaluo" name="cod_avaluo" value="<?php echo $cod_avaluo; ?>" readonly> 
...
<script>
    ...
    $("#push").click(function() {
        $(".preload, .load").show();
        var avaluo = <?php echo $cod_avaluo; ?>;
        var url5 = "<?php echo base_url("index.php/mcremate/VistaPrevia_Documento"); ?>";
        $("#resultado").load(url5, {cod_avaluo: avaluo}, function() {
            $(".preload, .load").hide();
        });
        //jQuery(".preload, .load").hide();
    });
    ...
    $('#imprimir').click(function() {
        ...
        switch (estado) {
            ...
            case "<?php echo TIPO_3; ?>" :
            case "<?php echo TIPO_6; ?>" :
            case "<?php echo TIPO_7; ?>" :
                tipo = '3';
                break;
            default:
                tipo = '2';
